...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		glm::vec3 lightPos = glm::vec3(2.0f, 0.0f, -0.5f);
		glm::vec3 lightCol = glm::vec3(0.9f, 0.85f, 0.5f);
		float     lightAmbientPow = 0.2f;
		float     lightSpecularPow = 1.0f;
		glm::vec3 ambientCol = glm::vec3(1.5f);
		...
		shader->SetUniform("u_AmbientAndSpecular", specAmbLight);
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Assignment #1"))
			{
				if (ImGui::Button("No Lighting!"))
				{
					...
					shader->SetUniform("u_AmbientAndSpecular", specAmbLight = 0);
				...
				if (ImGui::Button("Ambient + Specular!"))
				{
					...
					shader->SetUniform("u_Ambient", ambLight = 0);
					shader->SetUniform("u_Specular", specLight = 0);
					shader->SetUniform("u_AmbientAndSpecular", specAmbLight = 1);
					shader->SetUniform("u_AmbientSpecularToon", otherToon = 0);
				}
		...
		Texture2D::sptr rifleTexture = Texture2D::LoadFromFile("images/bluePng.png");
		Texture2D::sptr shotgunTexture = Texture2D::LoadFromFile("images/Yellow.jpg");
		...
		material0->Set("s_Diffuse", rifleTexture);
		material0->Set("s_Diffuse2", rifleTexture);
		...
		material2->Set("s_Diffuse", diffuse2);
		...
		ShaderMaterial::sptr shotgunMaterial = ShaderMaterial::Create();
		shotgunMaterial->Shader = shader;
		shotgunMaterial->Set("s_Diffuse", shotgunTexture);
		shotgunMaterial->Set("s_Diffuse2", shotgunTexture);
		shotgunMaterial->Set("s_Specular", specular);
		shotgunMaterial->Set("u_Shininess", 8.0f);
		shotgunMaterial->Set("u_TextureMix", 0.5f);
		...
		*/
		...
		GameObject obj1 = scene->CreateEntity("Plane");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Plane2.obj");
			obj1.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material2);
			obj1.get<Transform>().SetLocalPosition(2.0f, 0.0f, -1.0f);
			obj1.get<Transform>().SetLocalRotation(90.0f, 0.0f, 270.0f);
			obj1.get<Transform>().SetLocalScale(1.5f, 1.5f, 1.5f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj1);
		}

		
		GameObject obj2 = scene->CreateEntity("RifleGun");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/RifleGun.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material0);
			obj2.get<Transform>().SetLocalPosition(-1.0f, -1.0f, -0.5f);
			obj2.get<Transform>().SetLocalRotation(90.0f, 0.0f, 270.0f);
			obj2.get<Transform>().SetLocalScale(0.5f, 0.5f, 0.5f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}

		GameObject obj3 = scene->CreateEntity("ShotgunProtonFinal");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/ShotgunProtonFinal.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(shotgunMaterial);
			obj3.get<Transform>().SetLocalPosition(4.0f, -1.35f, -0.5f);
			obj3.get<Transform>().SetLocalRotation(90.0f, 0.0f, 360.0f);
		...
		{
			// Build a mesh
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/ProtonBlade.obj");
		...
		GameObject obj6 = scene->CreateEntity("following_monkey");
		{
			...
			// Set up a path for the object to follow
			pathing->Points.push_back({ 3.0f, 4.0f, 1.0f});
			pathing->Points.push_back({ 3.0f, 4.0f, 3.0f});
			pathing->Speed = 4.0f;
		}
